...
- 입력받은 횟수만큼 경주를 진행한다.
- 각 회차마다 모든 자동차의 이동 결과를 출력한다.
- 가장 멀리 이동한 자동차를 찾는다.
- 공동 우승자가 있으면 쉼표로 구분하여 출력한다.

## 예외 처리

- 자동차 이름이 올바르지 않으면 `IllegalArgumentException`을 발생시킨다.
- 시도 횟수가 숫자가 아니거나 올바르지 않으면 `IllegalArgumentException`을 발생시킨다.